...
struct InvalidArguments {
 ...
 std::string exchange_frame(std::string_view frame){
  ...
  bad.insert(pos+std::string("\"overrides\": {").size(),"\"unexpected\":true,");return inner.exchange_frame(bad);
 }
};
int main(int argc,char**argv){
 try{
  ...
  if(mode=="defaults"){
   ...
   if(mode=="same-value"){config::RunOverrides run;run.store=value.store;auto changed=client.read_with_overrides(run);require(changed.stamp==value.stamp&&changed.origins.store.rung=="environment","stamp follows provenance");}
  }
