"""Verify an installed C++ facade against three production Go services on Windows.

No arguments prints help. Uses short .build/facade paths, three unique local
pipes and separate service/client homes. No OS installation or C++ server is
...
def main():
    ...
    require({key for key, _ in compiler_identity} == {'ID', 'VERSION'}, 'native compiler identity not recorded')
    print('C++ compiler:', '; '.join(key + '=' + value for key, value in compiler_identity))
    ...
    try:
        ...
        print('PASS: installed facade reaches logging/config/router Go services; verified caller, provider values and audit')
    finally:
    ...
    print('PASS: service-owned state survives client exit; all five operations fail when services are absent; no client store')
