...
and builds a copied outside consumer using `find_package(abstraction_download_request)`.
...
This checks codec interoperability and package installation. The download CMake
package installs the header-only `abstraction_download_request` target alone,
with no job, CAS or IPC dependency. Codec-valid empty requests and
