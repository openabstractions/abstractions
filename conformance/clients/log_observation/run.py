"""Build a logging-only installed C++ SDK and exercise a private Go runtime."""
...
source_revision()
if a.dry_run: dry_run_stop('log_observation', a)
with tempfile.TemporaryDirectory(prefix='lo-',dir=build_root()) as temp:
 ...
 run([a.cmake,'-S',source,'-B',b/'sdk','-DABSTRACTION_FACADE_BUILD_AGGREGATE=OFF','-DABSTRACTION_FACADE_BUILD_LOGGING=ON','-DBUILD_SHARED_LIBS=OFF','-DABSTRACTION_IPC_BUILD_TESTS=OFF','-DCMAKE_BUILD_TYPE=Release'])
 ...
 run([exe,'--help']);env['OA_CPP_LOG_OBSERVER_PROBE']=str(exe)
 run(['go','test',*(['-race'] if a.race else []),'-count=1','-v',HERE/'fixture_test.go'])
print('PASS logging-only installed consumer; temporary prefix removed')
