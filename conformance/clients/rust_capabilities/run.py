"""Build outside Rust storage-changes and router clients and exercise an isolated Go runtime.
...
native library. Cargo's native link lines for the consumer must equal the
installed link-dependencies.txt.
"""
...
def main():
    ...
    p.add_argument("--run", action="store_true", help="build and run the Rust storage-changes/router consumer on this host (Windows or Linux)")
    ...
    source_revision()
    if a.dry_run: dry_run_stop('rust_capabilities', a)
    ...
    with tempfile.TemporaryDirectory(prefix="oa-rust-capabilities-") as tmp:
        b = Path(tmp); tree = b/"tree"
        for component, parts in [("identity", ["rust-frame"]), ("facade", ["rust", "rust-storage", "rust-router", "rust-model", "rust-logging", "rust-rights", "rs"]), ("storage", ["rust", "rs"]), ("router", ["rust", "rs"]), ("model", ["rust", "rs"]), ("download", ["rust", "rs"]), ("logging", ["rust", "rs"]), ("rights", ["rust", "rs"])]:
            for part in parts:
        ...
        for crate in ("rust-storage", "rust-router", "rust-model", "rust-logging", "rust-rights"):
        ...
        for component, part in [("identity", "rust"), ("facade", "rust-native")]:
            rel = Path("openabstractions-flat")/("abstraction-"+component)/part
            shutil.copytree(ROOT/rel, tree/rel)
        consumer = tree/"conformance/clients/rust_capabilities"; shutil.copytree(HERE, consumer)
        ...
        if build.returncode:
            # The output tail is often the failing rustc command line; show the diagnostics themselves.
            diagnostics = [l for l in (build.stdout + build.stderr).splitlines() if l.lstrip().startswith(("error", "-->", "|"))]
            sys.stderr.write("\n".join(diagnostics[:60]) + "\n")
        ...
        probe = b/("target/debug/capabilities-consumer"+(".exe" if windows else ""))
        run([probe, "--help"], env)
        env["OA_RUST_CAPABILITIES_PROBE"] = str(probe)
        run(["go", "test", *(["-race"] if a.race else []), "-v", "-count=1", HERE/"fixture_test.go"], env)
    print(f"PASS outside Rust storage-changes and router clients on {'Windows' if windows else 'Linux'}: observe/list refusals, read filtering, gap and snapshot, router refusal codes; temporary files removed")
