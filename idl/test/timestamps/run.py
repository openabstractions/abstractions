...
import("os";"fmt";"regexp";"strings";"encoding/json";rec "timestamp.test/generated/go/rec")
var micros=regexp.MustCompile(`^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}:[0-9]{2}:[0-9]{2}[.][0-9]{6}Z$`)
...
if !micros.MatchString(result)||micros.MatchString(p[1])!=(p[1]==result){panic("micros predicate")}
...
for line in open("cases.tsv"):
    ...
    try: r=rec.decode(base.replace(b"TOKEN",value.encode(),1))
        ...
        assert rec._micros_timestamp(result)
        assert rec._micros_timestamp(value)==(value==result)
        assert b"2026-09-11t12:00:00+02:00" in again.spec
        assert rec.encode(again)==encoded
    print(name+"\t"+result)
'''
sources["javascript"] = r'''import fs from 'node:fs';import * as rec from './generated/js/internal.mjs';
...
try{r=rec.decode(Buffer.from(base.replace('TOKEN',value)));}catch(e){if(!(e instanceof rec.Refusal))throw e;result=e.word;const v=rec.decode(Buffer.from(base));v.updatedAt=value;let bad=false;try{rec.encode(v)}catch(e){bad=e instanceof rec.Refusal&&e.word==='bad_timestamp'}if(!bad)throw Error('invalid writer accepted');}
if(r){const encoded=rec.encode(r),again=rec.decode(encoded);result=again.updatedAt;if(!rec.microsTimestamp(result)||rec.microsTimestamp(value)!==(value===result))throw Error('micros predicate');if(!Buffer.from(again.spec).toString().includes('2026-09-11t12:00:00+02:00'))throw Error('opaque altered');if(!Buffer.from(encoded).equals(Buffer.from(rec.encode(again))))throw Error('non-idempotent');}
...
Ok(r)=>{let encoded=rec::encode(&r);let again=rec::decode(&encoded).unwrap();assert!(rec::internal::micros_timestamp(&again.updated_at));assert_eq!(rec::internal::micros_timestamp(p[1]),p[1]==again.updated_at);assert!(String::from_utf8_lossy(&again.spec).contains("2026-09-11t12:00:00+02:00"));assert_eq!(encoded,rec::encode(&again));again.updated_at}};println!("{}\t{}",p[0],result);}}
'''
...
try{auto r=rec::decode(input);auto encoded=rec::encode(r);auto again=rec::decode(encoded);result=again.updated_at;assert(rec::detail::micros_timestamp(result));assert(rec::detail::micros_timestamp(value)==(value==result));assert(again.spec.find("2026-09-11t12:00:00+02:00")!=std::string::npos);assert(rec::encode(again)==encoded);}
