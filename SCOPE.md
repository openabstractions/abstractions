...
What this vocabulary names, which layers qualify for it, and in what order.
...
seat one. This vocabulary exists because nothing served seats two and three.
...
- *which app is asking* — **Kind B**. Built: `abstraction-identity`, Windows
  named pipes today.
...
> A layer enters this vocabulary only if seat two or seat three is missing, and
...
A vocabulary earns adoption by being small. SLF4J is one interface with five
methods. Ten half-built layers destroy the only thing on offer here, because
each one is a place a binding leaked in while attention was elsewhere.
...
By that rule no new layer has been earned: the ComfyUI node and the Lemonade
fork are both ours. Five layers have been added since the rule was written on
2026-09-05 — `cas`, `watch`, `identity`, `asks`, `rights` — each forced by a
failure in our own products. Nothing enforces this rule (`METHOD.md` §8) and
what was built contradicts it; whether the rule or the practice changes is the
owner's decision.

## 5. Order, when the rule is satisfied

The three that were next when this file was written — service discovery,
keep-awake, caller identity — are now `abstraction-config`,
`abstraction-rights` and `abstraction-identity`. Caller identity is the first
layer whose facade exposes a difference in fidelity rather than hiding it:
user and process are authoritative everywhere; verified *application* identity
only on macOS and for packaged apps on Windows.

Next, and only with an adopter demanding it: *what may this app do here*.
...
> **You may not add a layer you have not been forced into by a real case — an
> adopter, or our own products using the layer (`METHOD.md` §5).**
...
| `cas` | Go, Python, C++ | six writers, one file, no lost update; macOS `UNPROVEN` |
| `watch` | Go, Python, C++ | |
| `config` | Go | |
| `logging` | Go | |
| `model` | Go, Python | thinnest; least exercised |
| `identity` | Go | Windows named pipes today |
| `asks` | Go | |
| `rights` | Go | awake hold verified on Windows and Linux; macOS `UNPROVEN` |
| `facade` | Go | one entry point |
