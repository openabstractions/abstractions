...
if [ "$NROW" = 0 ]; then
    nothing "footprint rows" 0 1
elif [ -s "$RULES/fp-opaque-new" ]; then
    fail "footprint rows ($(wc -l < "$RULES/fp-opaque-new") new rows cleanup cannot act on)"
    detail "$RULES/fp-opaque-new"
...
if ! command -v powershell >/dev/null 2>&1; then
    skip "footprint claims — $(wc -l < "$RULES/fp-ask") rows name Windows locations and there is no powershell on PATH"
else
...
if [ "$NLIVE" = 0 ]; then
    skip "machine names — none of the $(wc -l < "$RULES/hosts") resolves, so this machine is off the network"
else
...
if [ -s "$RULES/who-bad" ]; then
    ...
    detail "$RULES/who-off"
else
    pass "somebody answers — the policy names what publishes, and every report address that was asked for exists"
fi

# check.sh eval's this file and reads this line as the proof it got here. A
# parse error empties the section and returns into nothing without it.
DOCUMENTS_RAN=1
