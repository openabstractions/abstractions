...
|---|---|---|
| `abstractions` | the charter: method, scope, results, the conformance corpus and its harness. A test suite, not a library. | `abstractions` |
| `abstraction-*` | one layer: the interface, in every language that implements it | `abstraction-job` |
| `abstraction-*-over-*` | a binding of one layer over an engine the adopter already has; ours is `-over-native`, one among peers. Decided 2026-09-07; none published yet. | `abstraction-download-over-curl` |
| `addon-*` | an optional backend for a layer, installed on another machine | `addon-synology` |
